...
public class AttributeSum extends ATerminalExpression implements IAttributeAggregate {
    ...
    public AttributeSum(Name attributeName) {
        this.attributeName = attributeName;
    }

    @Override
    public String getName() {
        return "sum(" + attributeName + ")";
    }
    ...
    public Result<IExpression> translate(List<IFormula> formulas, List<?> values) {
        ...
        for (Object value : values) {
            termList.add(new IfThenElse(iterator.next(), new Constant(type.cast(value), type), defaultValue));
        }
