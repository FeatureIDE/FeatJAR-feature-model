...
import de.featjar.feature.model.analysis.ComputeUniformity;
...
public class PrintSampleStatistics extends ACommand {
	...
	public LinkedHashMap<String, Object> collectStats(BooleanAssignmentList boolList, IFeatureModel model) {
        ...
        data.put("Number of Configurations",
        		Computations.of(boolList).map(ComputeNumberConfigurations::new).compute());
        data.put("Number of Variables",
        		Computations.of(boolList).map(ComputeNumberVariables::new).compute());
        data.put("Distribution of feature selection",
        		Computations.of(boolList).map(ComputeDistributionFeatureSelections::new).compute());
        data.put("Feature counter",
        		Computations.of(boolList).map(ComputeFeatureCounter::new).compute());
        data.put("Uniformity",
        		Computations.of(model).map(ComputeUniformity::new).set(ComputeUniformity.BOOLEAN_ASSIGNMENT_LIST, boolList).compute());
        return data;
	}
	/**
